...
async function executeRun(input: {
  ...
  try {
    ...
    if (input.spec.tools?.knowledgeSearch && input.spec.knowledgeBaseIds?.length) {
      ...
      const results = await searchKnowledgeContext({ userId: input.userId, runId: input.runId, query: input.inputText, knowledgeBaseIds: input.spec.knowledgeBaseIds, limit });
    ...
    const account = await getProviderAccountForRuntime(input.spec.providerAccountId!, input.userId);
