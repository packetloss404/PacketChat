...
export async function POST(request: Request) {
  ...
  if (!invite) return jsonError("Invite token is invalid or expired", 404);
  ...
  const userRows = await sql.begin(async (tx) => {
    ...
    await tx`update invite_tokens set consumed_by = ${users[0]!.id}, consumed_at = now() where id = ${invite.id}`;
    return users;
  });
